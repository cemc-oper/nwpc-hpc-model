...
from nwpc_hpc_model.slurm import SlurmQueryModel
from nwpc_hpc_model.workload_scheme import QueryCategory, QueryCategoryList, record_parser, value_saver
...
class TestSinfoQueryInfo(object):
    ...
    def test_to_dict(self):
        ...
        with open(nwp_output) as nwp_output_file:
            ...
            model = SlurmQueryModel.build_from_category_list(lines, category_list)
            model_dict = model.to_dict()
            assert('items' in model_dict is True)
            assert(len(model_dict['items']) == 3)
    ...
    def check_build_model(self, test_case):
        ...
        model = SlurmQueryModel.build_from_category_list(record, category_list)
